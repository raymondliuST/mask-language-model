...
class BERTTrainer:
    ...
    def accuracy(self, predictions, input):
        pred = torch.argmax(predictions, dim=2) #256xseq_length

        batch_size, seq_len = input["mlm_label"].shape

        pred_value_top_5, pred_labels_top_5 = torch.topk(predictions, 5, dim=2) # returns the top 10 predicted items
        correct_1 = 0
        correct_3 = 0
        correct_5 = 0
        total = 0
        ...
        for b in range(batch_size):
            ...
            true_labels = input["mlm_label"][b]

            b_pred_labels_top_5 = pred_labels_top_5[b]
        ...
        try:
            for epoch in range(hp.epochs):
                ...
                for i, data in data_iter:
                    ...
                    # accuracy
                    hit_1, hit_3, hit_5 = self.accuracy(mask_lm_output, data)
                    running_hit1.append(hit_1)
                    running_hit3.append(hit_3)
                    running_hit5.append(hit_5)
                    ...
                    post_fix = {
                        ...
                        # "avg_loss": avg_loss,
                        "loss": loss.item(),
                        "training_hit_1": hit_1,
                        "training_hit_3": hit_3,
                        "training_hit_5": hit_5,
                    }
                    ...
                    if self.step % hp.save_runs == 0 and data["mlm_input"].size(0) == hp.batch_size: # 不足batch数量则不采样
                        ...
                        # write model_param
                        for name, param in self.model.named_parameters():  # param.clone().cpu().data.numpy()   .module
                ...
                valid_loss, valid_avg_hit1, valid_avg_hit3, valid_avg_hit5 = self.evaluate(epoch, valid_writer)
                train_avg_hit1 = np.average(running_hit1)
                train_avg_hit3 = np.average(running_hit3)
                train_avg_hit5 = np.average(running_hit5)
                epoch_log = {
                        "epoch": epoch,
                        "avg_loss": avg_loss,
                        "train_avg_hit1": train_avg_hit1,
                        "train_avg_hit3": train_avg_hit3,
                        "train_avg_hit5": train_avg_hit5,
                        "valid_avg_hit1": valid_avg_hit1,
                        "valid_avg_hit3": valid_avg_hit3,
                        "valid_avg_hit5": valid_avg_hit5,
                    }
                wandb.log(epoch_log)
                print(f"EP_{epoch}, train_avg_loss={avg_loss}, valid_avg_loss={valid_loss}")
                print(f"EP_{epoch}, train_avg_acc={train_avg_hit1}, valid_avg_acc={valid_avg_hit1}")
    ...
    def evaluate(self, epoch, valid_writer):
        ...
        with torch.no_grad():
            ...
            return avg_loss, np.average(running_hit1), np.average(running_hit3), np.average(running_hit5)
